...
export class Icon2 extends PIXI.Sprite{
	...
	constructor(data:GameItemType) {
		...
		const fon = this.cont.addChild(new PIXI.Sprite(PIXI.Texture.from("images/icon2.png")));
		fon.x = -6;
		fon.y = -6;
		//
		...
		this.piccont.mask = this.cont.addChild(new PIXI.Graphics()).beginFill(0x0000ff, 1).moveTo(4,35)
			...
			.lineTo(342,296).lineTo(337,309).lineTo(323,320).lineTo(174,332).lineTo(19,320).lineTo(10,309).lineTo(2.8,298).lineTo(4,35)
			.endFill();
		this.piccont.addChild(new PIXI.Sprite(imagesLoader.resources[String(data.img)].texture));
		//
		const json0 = PIXI.Loader.shared.resources["images/anim/a2.json"].spritesheet;
		const array0:any = [];
		if(json0) {
